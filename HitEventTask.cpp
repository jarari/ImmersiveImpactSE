		...
		if (!ae || ae->magnitude >= ConfigManager::GetConfig()[iConfigType::StaggerLimit].value) {
			...
			if (cmd) {
				...
				if (ConfigManager::GetConfig()[iConfigType::EnableKnockback].value) {
					...
					vel *= ConfigManager::GetConfig()[iConfigType::Knockback_Fist + wepType].value;
					float dur = ConfigManager::GetConfig()[iConfigType::Knockback_Fist_Duration + wepType].value;
					if (ae->magnitude == ConfigManager::GetConfig()[iConfigType::StaggerLimit].value)
						PhysicsManager::SetVelocity(target, vel, hkVector4(), dur * ConfigManager::GetConfig()[iConfigType::Knockback_LastMultiplier].value);
					else
						PhysicsManager::SetVelocity(target, vel, hkVector4(), dur);
				}
			}
